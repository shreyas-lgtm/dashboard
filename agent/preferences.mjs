...
export const preferences = {
  ...
  tracks: {
    residential: {
      ...
      maxRent: 4200,           // $/mo — above this is flagged ABOVE_BUDGET
      idealRent: 3800,         // $/mo — best score at or below this
      maxSalePrice: 1_200_000, // $ — for "for sale" residential
      minBeds: 3,
      minBaths: 1,
      // Neighbourhoods / cities you actually want (case-insensitive substring).
      targetLocations: ['brooklyn', 'bedford-stuyvesant', 'bed-stuy'],
  ...
  weights: {
    budgetFit: 0.30,       // how comfortably it sits under budget
    bedsOrSizeFit: 0.20,   // beds (residential) or sqft (warehouse)
    locationMatch: 0.20,   // is it where you want
    pricePerSqft: 0.15,    // value for money (only when sqft known)
    freshness: 0.05,       // newer is better
    dataCompleteness: 0.10,// fully-specced listings rank above vague ones
  },
